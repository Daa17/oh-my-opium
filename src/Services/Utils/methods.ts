...
export const makeApprove = async (
  ...
  const tokenAddress =
    marginAddress || (await stakingContract?.methods.underlying().call());
  const tokenContract = createTokenContractInstance(tokenAddress);

  // Make allowance
  const tx = tokenContract?.methods
    .approve(poolAddress, MAX_UINT256)
...
export const checkTokenBalance = async (
  ...
  try {
    ...
    const tokenContract = createTokenContractInstance(tokenAddress);
    const balanceBN = await tokenContract?.methods
...
export const checkStakedBalance = async (
  ...
  try {
    ...
    const balanceBN = await stakingContract?.methods
      .calculateSharesToUnderlyingRatio(shares)
      .call();
    const decimals = await stakingContract?.methods.decimals().call();
    const balance = +convertFromBN(balanceBN, +decimals);
    return value > balance;
...
export const checkAllowance = async (
  ...
  // Create contracts instances
  const stakingContract = createReadOnlyStakingContractInstance(poolAddress);
  const tokenAddress = await stakingContract?.methods
    .underlying()
...
export const stakeIntoPool = async (
  ...
  const tx = stakingContract?.methods
    .deposit(convertToBN(value, +decimals))
    .send({ from: userAddress });
...
export const unstakeFromPool = async (
  ...
  const decimals = await stakingContract?.methods.decimals().call();
  const tx = stakingContract?.methods
    .withdraw(convertToBN(value, +decimals))
    .send({ from: userAddress });
...
export const buyProduct = async (
  ...
  // Send tx
  const tx = stakingContract?.methods
    .hedge(quantity, convertToBN(premium * 1.05, decimals))
    .send({ from: userAddress });
  return await sendTx(tx, onConfirm, onError);
};

export const getPremium = async (value: number, pool: PoolType) => {
  const { poolAddress, nominal } = pool;
  let premium;
  const stakingContract = createReadOnlyStakingContractInstance(poolAddress);

  const quantity = Math.floor(value / nominal);
  const { availableQuantity } = await stakingContract?.methods
  ...
  const decimals = await stakingContract?.methods.decimals().call();
  const premiumBN = await stakingContract?.methods
    .getRequiredPremium(quantity)
    .call();
  premium = +convertFromBN(premiumBN, decimals);
  return premium;
};

export const getInsurancePrice = async (value: number, pool: PoolType) => {
  const { nominal } = pool;
...
export const getPurchasedProducts = async (
  ...
  const stakingContractReadOnly =
    createReadOnlyStakingContractInstance(poolAddress);
  const stakingContract = createStakingContractInstance(poolAddress);
  ...
  if (dbPositions) {
    for (let pos of dbPositions) {
      ...
      const balance = await wrapperContract?.methods
        .balanceOf(userAddress)
        .call();
      if (balance !== "0") {
        const modifiedBalance: PositionType = {
          balance: +convertFromBN(balance, decimals),
          address,
          endTime,
        };
        ...
        .then(async (events) => {
          const balances: {
            ...
            events.map(async (event) => {
              const wrapperContract = createTokenContractInstance(
                event.returnValues.wrapper
              );
              const balance = await wrapperContract?.methods
                .balanceOf(userAddress)
                .call();
              return {
                balance,
                address: event.returnValues.wrapper,
          ...
          const modifiedBalances: {
            ...
            .map((el) => ({
              ...el,
              balance: +convertFromBN(el.balance, decimals),
            }));

          // Get endTime
          const finalizedBalances: PositionType[] = await Promise.all(
            modifiedBalances.map(async (el) => {
              const der = await stakingContractReadOnly?.methods
                .derivative()
                .call(undefined, el.blockNumber + 1);
...
export const getPurchasedProductsTheGraph = async (
  ...
  try {
    const response = await fetchTheGraph(
      subgraphs[authStore.networkId],
      `
        {
    ...
    const eventPositions: [{ endTime: string; longPositionAddress: string }] =
      response.longPositionTokens;
    const balances: { balance: string; address: string; endTime: number }[] =
      await Promise.all(
        eventPositions.map(async (event) => {
          const wrapperContract = createTokenContractInstance(
            event.longPositionAddress
          );
          const balance = await wrapperContract?.methods
            .balanceOf(userAddress)
            .call();
          return {
            balance,
            address: event.longPositionAddress,
...
export const withdrawPosition = async (
  ...
  // Create contracts instances
  const positionContract = createOpiumIERC20PositionContractInstance(
    position.address
  );
...
export const getStakedBalance = async (
  ...
  try {
    ...
    return await contract?.methods
      .balanceOf(userAddress)
      .call()
      .then((userShares: string) => {
        return contract?.methods
          .calculateSharesToUnderlyingRatio(userShares)
          .call()
          .then((shares: string) => {
            return contract?.methods
              .decimals()
...
export const getPoolPhase = async (poolAddress: string) => {
  ...
  const stakingPhaseLength = await stakingContract?.methods
    .STAKING_PHASE()
    .call();
  const tradingPhaseLength = await stakingContract?.methods
    .TRADING_PHASE()
    .call();
  let idleStakingTimeLimit = 0;
  try {
    idleStakingTimeLimit = await stakingContract?.methods
      .idleStakingTimeLimit()
      .call();
  } catch {
    idleStakingTimeLimit =
      +epochLength - +stakingPhaseLength - +tradingPhaseLength;
  }
  ...
  return {
    ...
    stakingPhase: `${convertDateFromTimestamp(
      phaseInfo.stakingStart,
      format
    )} - ${convertDateFromTimestamp(phaseInfo.stakingEnd, format)}`,
    tradingPhase: `${convertDateFromTimestamp(
      phaseInfo.tradingStart,
      format
    )} - ${convertDateFromTimestamp(phaseInfo.tradingEnd, format)}`,
    notInitialized: `${convertDateFromTimestamp(
      phaseInfo.notInitializedStart,
      format
    )} - ${convertDateFromTimestamp(phaseInfo.notInitializedEnd, format)}`,
    stakingOnly:
      phaseInfo.stakingOnlyStart === phaseInfo.stakingOnlyEnd
        ? ""
        : `${convertDateFromTimestamp(
            phaseInfo.stakingOnlyStart,
            format
          )} - ${convertDateFromTimestamp(phaseInfo.stakingOnlyEnd, format)}`,
  };
};
...
export const wrapToWopium = async (
  ...
  const wrapperContract = createWrapperContractInstance(
    wrapperProducts.wopium.poolAddress
  );
...
export const unwrapToOpium = async (
  ...
  const wrapperContract = createWrapperContractInstance(
    wrapperProducts.opium.poolAddress
  );
  const valueBN = convertToBN(value, wrapperProducts.opium.decimals);
  const tx = wrapperContract?.methods
    .unwrap(valueBN)
    .send({ from: userAddress });
...
export const getOpiumBalance = async (
  marginAddress: string,
  userAddress: string,
  decimals: number
) => {
...
export const getWopiumBalance = async (
  tokenMangerAddress: string,
  userAddress: string,
  decimals: number
) => {
  const tokenManagerContract =
    createTokenManagerContractInstance(tokenMangerAddress);
  const balanceBN = await tokenManagerContract?.methods
    .spendableBalanceOf(userAddress)
    .call();
  return +convertFromBN(balanceBN, decimals);
};
...
export const executeLong = async (
  ...
  const stakingContract = createReadOnlyStakingContractInstance(poolAddress);
  const longPositionWrapper = await stakingContract?.methods
    .longPositionWrapper()
    .call();
  const longPositionTokenContract =
    createOpiumIERC20PositionContractInstance(longPositionWrapper);

  const derivative = await stakingContract?.methods.derivative().call();

  let derivativeParams = ["950000000000000000", "2500"];
  try {
    derivativeParams = await stakingContract?.methods
      .getDerivativeParams()
      .call();
  } catch (e) {}
  ...
  const tx = longPositionTokenContract?.methods
    .execute(executeArgs)
    .send({ from: userAddress });
